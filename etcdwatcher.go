...
func main() {
	...
	flag.StringVar(&endpoint, "endpoint", "https://192.168.10.78:2379", "Etcd endpoint.")
...
func findPods(client *clientv3.Client, namespace string, key string, ctx context.Context, c chan podsInfo) {
	...
	modRev, _ := getBuildModRev(client, buildConf, ctx)
	...
	info = podsInfo{modRev: modRev, pods: pods}
	c <- info
}

//Support function. This functions retrieves the buildConfig's modRev to define the revision
// from which to start the observations.
func getBuildModRev(client *clientv3.Client, key string, ctx context.Context) (int64, error) {
...
func watch(client *clientv3.Client, pod string, ctx context.Context, c chan bool) {
	fmt.Println("First checking if pod is already ready...")
	if already, modRev := checkKey(client, pod); already == "True" {
		fmt.Println("The pod is already ready.")
		c <- true
	} else {
		fmt.Println("The pod is not ready yet. It is necessary to watch.")
